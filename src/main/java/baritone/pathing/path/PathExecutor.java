...
public class PathExecutor implements IPathExecutor, Helper {
    ...
    public boolean onTick() {
        ...
        if (possiblyOffPath(status, MAX_DIST_FROM_PATH)) {
            ticksAway++;
            logDebug("Far away from path for " + ticksAway
        ...
        if (end - start > 0) {
            logDebug("Recalculating break and place took "
                    + (end - start) + "ms");
        }*/
        if (pathPosition < path.movements().size() - 1) {
            IMovement next = path.movements().get(pathPosition + 1);
            if (!behavior.blockStateInterface().worldContainsLoadedChunk(next.getDest().x, next.getDest().z)) {
                ...
                if (MovementHelper.canUseFrostWalker(ctx, next.getDest().below())) {
                    ...
                    if (next instanceof MovementTraverse || next instanceof MovementParkour) {
                        boolean couldPlaceInstead = Baritone.settings().allowPlace.value
                                && behavior.baritone.getInventoryController().hasGenericThrowaway()
    ...
    private void cancel() {
        clearKeys();
        behavior.baritone.getInputController().setBlockBreakTarget(null);
    ...
    public boolean isSafeToCancel() {
        return pathPosition >= path.movements().size()
                || path.movements().get(pathPosition).safeToCancel();
    }
