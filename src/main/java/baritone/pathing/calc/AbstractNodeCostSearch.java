...
public abstract class AbstractNodeCostSearch implements IPathFinder, Helper {
    ...
    /**
     * Written by the server thread and observed by the path worker. Upstream
     * relies on cross-thread cancellation; volatile is required on the
     * dedicated-server scheduler where no monitor is held by calculate0.
     */
                ...
                if (logInfo) {
                    if (COEFFICIENTS[i] >= 3) {
                        Helper.HELPER.logDebug(
                                "Path cost coefficient is greater than three");
                    }
                    Helper.HELPER.logDebug(
